...
---
Project 4 Team 2 alphabetically: Bogi Britschgi, Kalin Kluth, Mason Obegi, Connor Olmstead, Laura Pelayo Uribe
...
Benchmarking Data: The Benchmark Energy by Individual Building data was very inconsistent. To address this, we added columns of NA values to the years that did not have all of the variables as the other years. Next, we organized the variables for all years so that they matched. We then removed the columns of data containing too many null values to be useful for analysis. Lastly, we bound the years of data together into one dataset.
...
Benchmarking Data: After merging data into one file using rbind by year, we then removed the null values. This included columns containing too many null values for usefulness.
...
Bench_cleaned <- Bench_cleaned %>% mutate(NumberofBuildings = ifelse(is.na(NumberofBuildings), 1.144, NumberofBuildings))
Bench_cleaned <- Bench_cleaned %>% mutate(NumberofFloors = ifelse(is.na(NumberofFloors), 4.0, NumberofFloors))
Bench_cleaned <- Bench_cleaned %>% mutate(LargestPropertyUseTypeGFA = ifelse(is.na(LargestPropertyUseTypeGFA), 83856, LargestPropertyUseTypeGFA))
Bench_cleaned <- Bench_cleaned %>% mutate(SiteEUI.kBtu.sf. = ifelse(is.na(SiteEUI.kBtu.sf.), 86.0, SiteEUI.kBtu.sf.))
Bench_cleaned <- Bench_cleaned %>% mutate(SiteEUIWN.kBtu.sf. = ifelse(is.na(SiteEUIWN.kBtu.sf.), 86.0, SiteEUIWN.kBtu.sf.))
Bench_cleaned <- Bench_cleaned %>% mutate(SourceEUI.kBtu.sf. = ifelse(is.na(SourceEUI.kBtu.sf.), 153.3, SourceEUI.kBtu.sf.))
Bench_cleaned <- Bench_cleaned %>% mutate(SourceEUIWN.kBtu.sf. = ifelse(is.na(SourceEUIWN.kBtu.sf.), 153.4, SourceEUIWN.kBtu.sf.))
Bench_cleaned <- Bench_cleaned %>% mutate(SiteEnergyUse.kBtu. = ifelse(is.na(SiteEnergyUse.kBtu.), 1.300e+07, SiteEnergyUse.kBtu.))
Bench_cleaned <- Bench_cleaned %>% mutate(SiteEnergyUseWN.kBtu. = ifelse(is.na(SiteEnergyUseWN.kBtu.), 1.280e+07, SiteEnergyUseWN.kBtu.))
Bench_cleaned <- Bench_cleaned %>% mutate(Electricity.kWh. = ifelse(is.na(Electricity.kWh.), 1034023, Electricity.kWh.))
Bench_cleaned <- Bench_cleaned %>% mutate(Electricity.kBtu. = ifelse(is.na(Electricity.kBtu.), 3584336, Electricity.kBtu.))
Bench_cleaned <- Bench_cleaned %>% mutate(TotalGHGEmissions = ifelse(is.na(TotalGHGEmissions), 129.5, TotalGHGEmissions))
Bench_cleaned <- Bench_cleaned %>% mutate(GHGEmissionsIntensity = ifelse(is.na(GHGEmissionsIntensity), 1.186, GHGEmissionsIntensity))
...
Bench_cleaned <- Bench_cleaned %>%
  mutate(
    NumberofBuildings = ifelse(is.na(NumberofBuildings), 1.144, NumberofBuildings),
    NumberofFloors = ifelse(is.na(NumberofFloors), 4.0, NumberofFloors),
    LargestPropertyUseTypeGFA = ifelse(is.na(LargestPropertyUseTypeGFA), 83856, LargestPropertyUseTypeGFA),
    `SiteEUI.kBtu.sf.` = ifelse(is.na(`SiteEUI.kBtu.sf.`), 86.0, `SiteEUI.kBtu.sf.`),
    `SiteEUIWN.kBtu.sf.` = ifelse(is.na(`SiteEUIWN.kBtu.sf.`), 86.0, `SiteEUIWN.kBtu.sf.`),
    `SourceEUI.kBtu.sf.` = ifelse(is.na(`SourceEUI.kBtu.sf.`), 153.3, `SourceEUI.kBtu.sf.`),
    `SourceEUIWN.kBtu.sf.` = ifelse(is.na(`SourceEUIWN.kBtu.sf.`), 153.4, `SourceEUIWN.kBtu.sf.`),
    SiteEnergyUse.kBtu. = ifelse(is.na(SiteEnergyUse.kBtu.), 1.300e+07, SiteEnergyUse.kBtu.),
    SiteEnergyUseWN.kBtu. = ifelse(is.na(SiteEnergyUseWN.kBtu.), 1.280e+07, SiteEnergyUseWN.kBtu.),
    Electricity.kWh. = ifelse(is.na(Electricity.kWh.), 1034023, Electricity.kWh.),
    Electricity.kBtu. = ifelse(is.na(Electricity.kBtu.), 3584336, Electricity.kBtu.),
    TotalGHGEmissions = ifelse(is.na(TotalGHGEmissions), 129.5, TotalGHGEmissions),
    GHGEmissionsIntensity = ifelse(is.na(GHGEmissionsIntensity), 1.186, GHGEmissionsIntensity)
  )

colSums(is.na(Bench_cleaned))
...
Benchmarking Data: 
To address NA values in the numerical fields, we substituted the column means, taken from summary data, to replace NULL values. We left the categorical variable NA values alone for now.
...
#we are doing 3 checks to see if every column in every year is in 2022, 2016 and 2018 respectively as I was having issues with incompatible dataset before
...
In the PSE and equity data cleaning process, we prioritized data integrity, ensuring the accuracy and consistency of our datasets throughout their life cycle. This process began with the crucial step of renaming columns to enhance clarity and consistency, thereby making the datasets more intuitive for analysis.
...
We wanted to test the theory that buildings get bigger as time goes on. We can see a positive correlation especially for the biggest buildings for each time period/year. 
...
The correlation matrix shows that there are some relationships we should explore further. For instance: Site Energy Use has a useful relationship with with greenhouse gas emission intensity and total greenhouse gas emissions is positively related to electricity use. 
...
The boxplots for the Energy Benchmarking per Building show that there are many outliers in the data set. The variation in this data is to be expected as skyscrapers will use more energy than one-story buildings. Some businesses will use large quantities of energy while others will not. The outliers will help us understand what the biggest contributors to energy use and greenhouse gas emissions are. We will look into these relationships in further analysis.
...
Histograms also visually portray the distribution of the data, showing that for most fields, the highest values are an exception to the norm.
...
# References
PSE Usage Data and shape files:
https://data-seattlecitygis.opendata.arcgis.com/datasets/SeattleCityGIS::pse-usage-data-for-ose-climate-portal/about

Racial and Social Equity Data and shape files:
https://data-seattlecitygis.opendata.arcgis.com/datasets/SeattleCityGIS::racial-and-social-equity-composite-index-current/aboutv

Building Performance Data and Energy Benchmarking Data:
https://www.seattle.gov/environment/climate-change/buildings-and-energy/energy-benchmarking/data-and-reports#summaryinformationbybuildingtype