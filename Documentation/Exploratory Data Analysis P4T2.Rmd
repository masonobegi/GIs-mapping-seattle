...
When using na.omit, it removes every row that has an NA value. The 2017 data is the only data set that has the Low/High Outlier cutoff columns, and for those columns almost every single row has an NA value. Therefore, we would have been only left with 3 rows for the whole 2017 dataset. Because of that, we deemed these columns unnecessary and removed them from the 2017 dataset. Omitting NA values allows for cleaner exploratory data analysis now even though we may revisit with these rows in the future.
...
We are renaming columns so they are all a unifrom name. THen we are checking to see if all columsn fit in the specific years (2022, 2016, 2018) to make sure all our data is uniform. We are then adding a year column and combining the data based on the rows. Then after that we are omitting the Citywide data because it is a majot outlier an dnot a building type we are interested in.
...
P-value being extremely small shows that the median ES score and the total GFA are not varying wildly
...
Wanted to test the theory that buildings get bigger as time goes on. Can see a positive correlation especially for the biggest buildings for each time period/year. 
...
Wanting to see how efficiently buildings are using their energy by build year
...
Visually showing what total GFA looks like for each different type of building in our dataset and which types of buildings are typically bigger than others. Y axis is GFA in tens of millions.
