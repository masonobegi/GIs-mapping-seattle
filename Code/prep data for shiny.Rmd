...
reduced_pse_2023<-reduced_pse_2023 %>% group_by(Sector) %>%
 mutate(reduction_fact = sum(EMISSIONSP) / total_sum * (1- 0.39))
...
```{r reduced 39% across}
reduced_pse_2023<-reduced_pse_2023 %>% mutate(EMISSIONSP_reduced_total = EMISSIONSP * (1-0.39))
...
# Cut the Emissions_ data into bins based on these percentiles
reduced_pse_2023$Emissions_Percentile <- cut(reduced_pse_2023$EMISSIONSP_reduced_total, 
                                           breaks = c(-Inf, quantiles, Inf), 
                                           include.lowest = TRUE, 
                                           labels = c("0-20%", "20-40%", "40-60%", "60-80%", "80-100%"))
...
ggplot() +
  ...
  labs(title = "Seattle Reduced Gas Emissions Per Capita for 2030 by total 39%") +
  theme_minimal() +
  theme(legend.position = "bottom") +
  coord_sf(datum = NA)
...
```{r}
# Now use this filtered dataset in your leaflet map
reducedPalette <- colorQuantile(c("#2dc937", "#99c140", "#e7b416","#db7b2b", '#cc3232'),
                                     domain = PSE2023$Emissions_Percentile,
                                     n = 5)
leaflet(PSE2023) %>%
  addTiles() %>%
  addPolygons(
    fillColor =~reducedPalette(Emissions_Percentile),
    weight = 2,
    opacity = 1,
    color = "white",
    dashArray = "3",
  ...
  addLegend(
    pal = c("0-20%" = "#2dc937", "20-40%" = "#99c140", "40-60%" = "#e7b416", "60-80%" = "#db7b2b", "80-100%" = '#cc3232'),5,
