...
#removing null values from the Calendar Year column
pse_tseries <- as.data.frame(pse_data)
pse_tseries <- pse_tseries %>% drop_na(CAL_YEAR)
sapply(pse_tseries, function(x) sum(is.na(x)))

```
This code selects a subset of relevant columns from the merged dataframe to reduce the number of parameters and only ones that we are likely to use remain. Then, it renames the columns because the abbreviations were unclear about what data is contained in the column. It checked for missing values in the columns but there were none. It also changes the Highest Priority level label to simply 'Highest' in the QUINTILE columns because they all had different values for it and this fits better into with the other labels that are also more simple, such as 'Lowest'. Finally, it removes null values from the initial pse_data dataset so that time series analysis can be performed. Missing values in the emission data columns was left because substituting with any number would skew results.
